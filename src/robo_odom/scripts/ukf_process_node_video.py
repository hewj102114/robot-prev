...
def UKFinit():
    ...
    ukf.Q[0:3, 0:3] = Q_discrete_white_noise(3, dt=dt, var=2)
    ukf.Q[3:6, 3:6] = Q_discrete_white_noise(3, dt=dt, var=2)
    ukf.P = np.diag([8, 1, 0.8, 5, 1, 0.8])
...
def callback_imu(imu):
    ...
    global pos_imu_x, pos_imu_y, pos_wheel_x, pos_wheel_y
    if imu_init == True:
        ...
        pos_imu_x = pos_imu_x + acc_imu_x * dt
        pos_imu_y = pos_imu_y + acc_imu_y * dt
        ...
        ukf_pos.pose.pose.position.x = ukf_out_pos_x
        ukf_pos.pose.pose.position.y = ukf_out_pos_y
        ...
        uwb_pos.header.stamp.secs = imu.header.stamp.secs
        uwb_pos.header.stamp.nsecs = imu.header.stamp.nsecs
        uwb_pos.pose.pose.position.x = pos_uwb_x
        uwb_pos.pose.pose.position.y = pos_uwb_y
        uwb_pos.pose.pose.orientation.x = 0
        uwb_pos.pose.pose.orientation.y = 0
        uwb_pos.pose.pose.orientation.z = 0
        uwb_pos.pose.pose.orientation.w = 1
        pub_uwb_pos.publish(uwb_pos)
        

        imu_pos = Odometry()
        imu_pos.header.frame_id = "odom"
        imu_pos.header.stamp.secs = imu.header.stamp.secs
        imu_pos.header.stamp.nsecs = imu.header.stamp.nsecs             
        imu_pos.pose.pose.position.x = pos_imu_x
        imu_pos.pose.pose.position.y = pos_imu_y
        imu_pos.pose.pose.orientation.x = imu.orientation.x
        imu_pos.pose.pose.orientation.y = imu.orientation.y
        imu_pos.pose.pose.orientation.z = imu.orientation.z
        imu_pos.pose.pose.orientation.w = imu.orientation.w
        pub_imu_pos.publish(imu_pos)
        
        wheel_pos = Odometry()
        wheel_pos.header.frame_id = "odom"
        wheel_pos.header.stamp.secs = imu.header.stamp.secs
        wheel_pos.header.stamp.nsecs = imu.header.stamp.nsecs       
        wheel_pos.pose.pose.position.x = pos_wheel_x
        wheel_pos.pose.pose.position.y = pos_wheel_y
        wheel_pos.pose.pose.orientation.x = 0
        wheel_pos.pose.pose.orientation.y = 0
        wheel_pos.pose.pose.orientation.z = 0
        wheel_pos.pose.pose.orientation.w = 1
        pub_wheel_pos.publish(wheel_pos)
...
pub_uwb_pos = rospy.Publisher('/demo/uwb/pos', Odometry, queue_size=1)
pub_wheel_pos = rospy.Publisher('/demo/wheel/pos', Odometry, queue_size=1)
pub_imu_pos = rospy.Publisher('/demo/imu/pos', Odometry, queue_size=1)
rospy.spin()
