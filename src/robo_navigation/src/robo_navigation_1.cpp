...
void RoboNav::cb_scan(const sensor_msgs::LaserScan::ConstPtr &scan)
{
    ...
    read_lidar=1;
    ROS_INFO("obs_min[1][0]: %f",obs_min[1][0]);
}
...
int main(int argc, char **argv)
{
    ...
    while (ros::ok())
    {
        ...
        if (read_lidar)
        {
            ...
            if (robo_nav.lidar_state < 4) //main control cannot get the end state signal
            {
                robo_nav.lidar_nav(msg_vel);
            }

            if (robo_nav.lidar_state == 4)
            {
